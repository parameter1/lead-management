...
const calculatePacing = ({
  ...
  const totalDays = moment(endDate).diff(moment(startDate), 'days') + 2;
  const requiredLeadsPerDay = requiredLeads / totalDays;
  const daysElapsed = now >= endDate ? totalDays : moment(endDate).diff(moment(), 'days') - 1;
...
const commonResolvers = {
  order: (lineItem) => Order.findById(lineItem.orderId),
  status: ({ range }) => {
